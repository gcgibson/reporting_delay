...
if False:
    ...
    ytrain = ytrain[:780]
    Xtrain = np.arange(1,len(ytrain)+1).reshape((-1,1))
    ...
    # Test data
    n_ahead = 40
    Xtest = np.arange(len(ytrain)+1,len(ytrain)+1+n_ahead).reshape((-1,1))
    n = len(Xtest)
    ...
    def mean_func(x):
        ...
        for elm in x:
            ...
            t = np.linspace(1, elm  , elm )
            # The SIR model differential equations.
            def deriv(y, t, N, beta, gamma):
                S, I, R = y
                #print (-beta*(1+10*np.cos(2*np.pi*t)))
                dSdt = -beta * S * I / N
...
for i in range(len(predictMean)):
    samples = np.random.normal(predictMean[i],np.sqrt(predictVar[i]),1000)
...
#state_trajectories = state_trajectories[:,:100]
weighted_trajectories = []
for state_traj in np.transpose(state_trajectories):
    weight = []
    for i in range(len(test_n_t_d)):
        p_sum = sum(phat[:len(phat)-1-i])
        tmp = binom.pmf(sum(test_n_t_d[i][:len(phat)-1-i]),state_traj[i],p_sum)
        weight.append(tmp)
    weighted_trajectories.append(weight)
        
weighted_trajectories = np.transpose(np.array(weighted_trajectories))


###
from sklearn.metrics import mean_squared_error

print (mean_squared_error(np.average(state_trajectories,axis=1,weights = weighted_trajectories),test_n_t_inf))
